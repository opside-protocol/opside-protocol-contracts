...
describe('Contract', () => {
    ...
    it('should regster slot', async () => {
        ...
        await openRegistrarContract.connect(deployerAdapter).request(name, admin.address, 182, 0, {value: ethers.utils.parseEther("1")});
    ...
    it('should accept slot 2', async () => {
        await openRegistrarContract.connect(deployerAdapter).request(name+'2', admin2.address, 182, 0, {value: ethers.utils.parseEther("1")});
        ...
        await openRegistrarContract.connect(deployerAdapter).request(name+'3', admin2.address, 182, 0, {value: ethers.utils.parseEther("1")});
    ...
    it('should check adapter distributeRewards', async() => {
        ...
        await minerDepositContract.connect(receiveAddress).deposit(0, {value: ethers.utils.parseEther('100')});
        ...
        let balance = await ethers.provider.getBalance(receiveAddress.address);
        ...
        await expect(minerDepositContract2.connect(receiveAddress).deposit(0, {value: ethers.utils.parseEther('100')})).to.be.rejectedWith('invalid deposit');
        await minerDepositContract2.connect(receiveAddress2).deposit(0, {value: ethers.utils.parseEther('100')});
        ...
        await openRegistrarContract.connect(deployerAdapter).request(name+'4', admin2.address, 182, 0, {value: ethers.utils.parseEther("1")});
        ...
        await expect(minerDepositContract3.connect(receiveAddress).deposit(0, {value: ethers.utils.parseEther('100')})).to.be.rejectedWith('invalid deposit');
    ...
    it('should check add and remove zero gas contract', async() => {
        ...
        console.log(await slotAdapterContract.getGasFreeContracts())
    ...
    // it('should check opsideSlot fundSlotRewards', async() => {
    //     await opsideSlotsContract.connect(deployerAdapter).fundSlotRewards(2, { value: 10 });
    //     let slot = await opsideSlotsContract.getSlot(2);
    //     expect(slot.rewardsPool).to.be.equal(10);

    //     await expect(opsideSlotsContract.connect(deployerAdapter).fundSlotRewards(1, { value: 10 })).to.be.rejectedWith('Deregister');
    // });

    // it('should check adapter setSlotManager', async() => {
    //     await expect(opsideSlotsContract.connect(admin2).setSlotManager(2, admin3.address)).to.be.rejectedWith('EOA');

    //     await expect(slotAdapterContract.connect(admin).changeSlotManager(admin.address)).to.be.rejectedWith('OnlySlotAdapter()');

    //     await expect(slotAdapterContract.connect(admin2).changeSlotManager(admin.address)).to.be.rejectedWith('OnlyManager()');

    //     expect(await slotAdapterContract2.slotManager()).to.be.equal(admin2.address);

    //     await slotAdapterContract2.connect(admin2).changeSlotManager(admin.address);

    //     expect(await slotAdapterContract2.slotManager()).to.be.equal(admin.address);

    //     await slotAdapterContract2.connect(admin).changeSlotManager(admin2.address);

    //     expect(await slotAdapterContract2.slotManager()).to.be.equal(admin2.address);
    // });

    // it('should check adapter pause', async() => {
    //     await slotAdapterContract2.connect(admin2).pause();
    //     expect(await slotAdapterContract2.status()).to.be.equal(3);
    // });

    // it('should check adapter unpause', async() => {
    //     await slotAdapterContract2.connect(admin2).unpause();
    //     expect(await slotAdapterContract2.status()).to.be.equal(2);
    // });

    // it('should check adapter stop', async() => {
    //     await slotAdapterContract2.connect(admin2).stop();
    //     expect(await slotAdapterContract2.status()).to.be.equal(4);
    // });
});
