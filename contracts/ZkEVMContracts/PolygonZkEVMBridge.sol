    ...
    IPolygonZkEVMBridge
{
    ...
    function bridgeAsset(
        ...
        if (destinationNetwork >= _SELF_ROLLUP_NETWORK_ID && bridgeCrossFee != 0) {
            fee = bridgeCrossFee;
        }
    ...
    function isCrossRollup(uint32 destinationNetwork) private view returns (bool) {
        return (networkID == _MAINNET_NETWORK_ID && destinationNetwork > _SELF_ROLLUP_NETWORK_ID);
    }
    ...
    function claimAsset(
        ...
        if (isCrossRollup(destinationNetwork)) {
            ...
            IPolygonZkEVMBridge(destinationL1BridgeAddress).bridgeAsset{value: transferAmount}(_SELF_ROLLUP_NETWORK_ID, destinationAddress, amount, destinationTokenAddress, true);
        }
    ...
    function setRollupIdInfoAddress(address _rollupIdInfoAddress) external onlyAdmin {
        rollupIdInfoAddress = _rollupIdInfoAddress;
    }

    function setBridgeCrossFee(uint256 _bridgeCrossFee) external onlyAdmin {
